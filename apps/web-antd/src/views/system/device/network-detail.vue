...
const bandwidthOption = computed(() => {
  ...
  return {
    title: {
      text: '24小时带宽趋势',
      left: 'center',
      textStyle: { fontSize: 13, fontWeight: 'normal', color: '#262626' },
    },
    tooltip: {
      trigger: 'axis',
    ...
    <div class="page-header">
      ...
      <div class="page-header-right">
        <div class="overview-card">
          <IconifyIcon icon="lucide:arrow-down" style="font-size: 16px; color: #1677ff;" />
          <div class="overview-info">
            <span class="overview-label">下载速度</span>
            <span class="overview-value" style="color: #1677ff;">{{ formatSpeed(net.downloadSpeed) }}</span>
          </div>
        </div>
        <div class="overview-card">
          <IconifyIcon icon="lucide:arrow-up" style="font-size: 16px; color: #52c41a;" />
          <div class="overview-info">
            <span class="overview-label">上传速度</span>
            <span class="overview-value" style="color: #52c41a;">{{ formatSpeed(net.uploadSpeed) }}</span>
          </div>
        </div>
        <div class="overview-card">
          <IconifyIcon icon="lucide:zap" style="font-size: 16px; color: #faad14;" />
          <div class="overview-info">
        ...
        <div class="overview-card">
          <IconifyIcon icon="lucide:git-compare" style="font-size: 16px; color: #722ed1;" />
          <div class="overview-info">
    ...
    <div class="detail-body">
      ...
      <Card class="info-card" :bordered="true" :body-style="{ padding: '20px' }">
        <div class="panel-title">
          <IconifyIcon icon="lucide:info" style="font-size: 14px;" />
          网络配置
        </div>
      ...
      <Card class="stats-card" :bordered="true" :body-style="{ padding: '16px' }">
        <div class="panel-title">
          <IconifyIcon icon="lucide:bar-chart-3" style="font-size: 14px;" />
          流量统计
        </div>
        <div class="stats-grid">
          <div class="stat-cell" style="border-left-color: #1677ff;">
            <span class="stat-label">今日下载</span>
            <span class="stat-value" style="color: #1677ff;">{{ net.totalDownloadToday }} MB</span>
          </div>
          <div class="stat-cell" style="border-left-color: #52c41a;">
            <span class="stat-label">今日上传</span>
            <span class="stat-value" style="color: #52c41a;">{{ net.totalUploadToday }} MB</span>
          </div>
          <div class="stat-cell" style="border-left-color: #722ed1;">
            <span class="stat-label">总接收量</span>
            <span class="stat-value">{{ formatBytes(net.rxBytes) }}</span>
          </div>
          <div class="stat-cell" style="border-left-color: #faad14;">
            <span class="stat-label">总发送量</span>
            <span class="stat-value">{{ formatBytes(net.txBytes) }}</span>
          </div>
          <div class="stat-cell" style="border-left-color: #ff4d4f;">
            <span class="stat-label">接收错误</span>
            <span class="stat-value" :style="{ color: net.rxErrors > 0 ? '#ff4d4f' : '#52c41a' }">
              {{ net.rxErrors }}
            </span>
          </div>
          <div class="stat-cell" style="border-left-color: #eb2f96;">
            <span class="stat-label">发送错误</span>
            <span class="stat-value" :style="{ color: net.txErrors > 0 ? '#ff4d4f' : '#52c41a' }">
              {{ net.txErrors }}
            </span>
          </div>
          <div class="stat-cell" style="border-left-color: #8c8c8c;">
