...
const folderInfo = ref<TeamFileFolder | null>(null);
const files = ref<TeamFolderFile[]>([]);
...
async function loadData() {
  ...
  try {
    const [foldersRes, filesRes] = await Promise.all([
      getTeamFileFoldersApi(),
      getTeamFolderFilesApi(folderId.value),
    ]);
    folderInfo.value = foldersRes.find((f) => f.id === folderId.value) || null;
...
function goBack() {
  router.push('/storage/team-files');
}

function getFileIcon(file: TeamFolderFile): string {
...
function openRenameModal(file: TeamFolderFile) {
...
function handleDeleteFile(file: TeamFolderFile) {
...
<template>
  <div class="team-folder-detail-page">
    <!-- ═══════ 页面顶部概览 ═══════ -->
    <div class="page-header">
      <div class="page-header-left">
        ...
        <div class="page-title-area">
          <h1 class="page-title">{{ folderInfo?.name || '加载中...' }}</h1>
          <p class="page-desc">团队文件 · {{ filteredFiles.length }} 个项目</p>
        </div>
      </div>
    ...
    <div class="file-content">
      ...
      <div v-else class="file-grid">
        ...
        <div v-if="sortedFiles.length === 0" class="file-grid-empty">
          暂无文件
        </div>
      </div>
    </div>
