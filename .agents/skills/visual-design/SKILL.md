...
全局统一的 Badge 徽标组件，由 **dot（小圆点）+ text（文本）** 组成，用于表达对象的状态、连接情况、健康度等轻量级语义信息。

与 Tag 的区别：
- **Tag** 用于分类、标签、属性标记，带背景色块，视觉较重；
- **Badge** 用于状态指示（如"已连接 / 未连接 / 运行中"等），仅以小圆点 + 文本呈现，视觉极轻，适合密集排版场景（如卡片右上角、列表行末、表头筛选项后）。

#### 尺寸规格

| 属性 | 值 | Token |
|------|-----|-------|
| 整体高度 | 由文本行高决定（无额外背景容器） | — |
| dot 直径 | 6px | — |
| dot 圆角 | 50% | — |
| dot 与文本间距 | 8px | `--ict-space-2` |
| 文本字号 | 14px / 400 | `--ict-body-medium` |
| line-height | 20px | — |

#### 语义配色

| 语义 | dot 颜色 | text 颜色 | 典型场景 |
|------|---------|----------|----------|
| `default` | `#C9CDD4` | `--ict-text-secondary` | 中性状态：未连接、未启用、无数据 |
| `danger` | `--ict-danger` | `--ict-text-primary` | 异常、错误、离线、严重告警 |
| `warning` | `--ict-warning` | `--ict-text-primary` | 警告、降级、需关注 |
| `success` | `--ict-success` | `--ict-text-primary` | 正常、已连接、运行中、健康 |
| `primary` | `--ict-primary` | `--ict-text-primary` | 进行中、处理中、品牌强调态 |
| `info` | `--ict-info` | `--ict-text-primary` | 信息提示、特殊标记 |

> **关键设计原则**：
> - `default` 是**唯一一种** dot 与 text 同时使用次级色（灰）的语义，整体呈"无强调"质感，用于表达"无需关注"的状态。
> - 其它 5 种语义统一使用 **彩色 dot + 主文字色 text** 的组合：以小圆点提供语义颜色，但文本保持 `--ict-text-primary` 的主体色，避免大面积彩色文本造成视觉污染。
...
|------|------|--------|------|
| `text` | `string` | — | 徽标文本 |
| `type` | `'default' \| 'primary' \| 'success' \| 'warning' \| 'danger' \| 'info'` | `'default'` | 徽标语义类型 |

#### 使用规范

1. **语义选择**：必须根据对象的真实状态选择对应 `type`，禁止以视觉好看为由随意搭配。
2. **文本精简**：徽标文本应尽量简短，建议不超过 4 个汉字（如"已连接 / 运行中 / 离线"）。
3. **优先使用 Badge 表达状态**：所有"状态指示"场景（已启用、运行中、已连接等）**必须使用 Badge 而非 Tag**，避免色块过重打破卡片节奏。
4. **组合使用**：多个 Badge 横向排列时（如"已连接 1 / 未连接 1"），项与项之间间距为 `20px`（`--ict-space-5`），dot 与文本之间间距固定为 `8px`（`--ict-space-2`）。
5. **可与数字组合**：徽标可在 text 后追加数字（如"已连接 1"），数字使用 `--ict-text-emphasis` + `font-weight: 500` 强调。
6. **禁止背景色**：Badge 绝对不能添加背景色块（含浅色背景），否则将与 Tag 混淆，违反组件分工。
7. **禁止点击**：Badge 为纯展示组件，不响应点击；如需"筛选切换"等交互，应使用按钮或 Tag 组件替代。
