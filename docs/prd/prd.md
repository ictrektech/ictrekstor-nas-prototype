...
**技术实现**：当前的存储池实现基于 mdadm 创建 md 设备，将 md 设备添加到 LVM 中，创建一个大的 VG（卷组），作为存储池。
...
**技术实现**：存储空间本质上是一个可以存储文件的挂载点。对于来自存储池的存储空间，实现时是在存储池的 VG 设备上划分 LV（逻辑卷），格式化成指定的文件系统后挂载出来；对于基于目录的存储空间，是将指定本地目录通过 bind mount 的方式挂载出来。
...
**定义**：指逻辑上不同 NAS 用户之间，将一个用户的某个子目录授权给其他用户访问，控制访问权限和有效期。

> **注意**：此处的"分享"特指用户之间的目录授权访问机制，以区别于通过网络协议访问时的"SMB 共享"、"NFS 共享"等概念，避免混淆。
...
**相关页面**：
- [我的分享](./file/shared.md) — 管理当前用户创建的所有分享
- [他人分享](./file/shared-from-others.md) — 浏览他人分享给当前用户的文件
